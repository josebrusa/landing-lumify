...
<template>
  <section id="retainers" class="py-[100px] px-[5%] bg-gray-dark">
    ...
    <div class="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6">
      <div
        ...
        :key="r.key"
        class="reveal rounded-radius p-9 py-8 border transition-all duration-300 relative"
        :class="r.best
      ...
      >
        ...
        <a
          href="#"
          class="block text-center py-3 rounded-full font-semibold text-[0.87rem] border transition-colors no-underline"
          :class="r.best ? 'border-blue text-blue' : 'border-white/20 text-white/70 hover:border-blue hover:text-blue hover:bg-blue/10'"
          @click.prevent="openModal(r.key)"
        >
          {{ t('ret.cta') }}
        </a>
      </div>
